...
class DeckEncoder {
  static decode(code) {
    ...
    if (format !== DeckEncoder.FORMAT) {
      throw new TypeError(
        "The provided code does not match the required format."
      );
    }
    ...
    while (bytes.length > 0) {
      ...
      result.push(
        Card.from(
          fourPlusSetString,
          fourPlusFactionString,
          fourPlusNumberString,
          fourPlusCount
        )
      );
    }
  ...
  static encode(cards) {
    ...
    const grouped3 = this.groupByFactionAndSetSorted(
      cards.filter((c) => c.count === 3)
    );
    const grouped2 = this.groupByFactionAndSetSorted(
      cards.filter((c) => c.count === 2)
    );
    const grouped1 = this.groupByFactionAndSetSorted(
      cards.filter((c) => c.count === 1)
    );
    const nOfs = cards.filter((c) => c.count > 3);

    return Base32.encode([
      (DeckEncoder.FORMAT << 4) |
        (cards.reduce((p, c) => Math.max(p, c.version), 0) & 0xf),
  ...
  static groupByFactionAndSetSorted(cards) {
    ...
    while (cards.length > 0) {
      ...
      for (let i = cards.length - 1; i >= 0; i--) {
        ...
        if (
          first.set === compare.set &&
          first.faction.id === compare.faction.id
        ) {
          set.push(compare);
          cards.splice(i, 1);
        }
