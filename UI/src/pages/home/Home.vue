<template>
  ...
  <div
    class="left-nav no-scrollbar overflow-y-scroll"
    :class="{ expanded: leftNavExpanded }"
  >
    <div
      class="left-nav-btn logo no-click"
      v-if="!IS_ELECTRON"
      @mouseenter="showAds"
    >
      <picture>
  ...
  >
    ...
    <div class="card square" @click="openURL('https://www.swimstrim.com/')">
      <img
        src="https://www.swimstrim.com/packs/media/images/logo-8b7cd382.png"
      />
    </div>
  ...
  <div
    class="deck-content-container"
    :class="{ hide: !isShowDeck, fullheight: !IS_ELECTRON }"
  >
  ...
  <div class="bottom-bar" v-if="IS_ELECTRON">
    <div class="left">
      ...
      <div
        v-if="!localApiEnabled && lorRunning && IS_ELECTRON"
        class="api-warning warning"
      >
        <i class="fas fa-exclamation-triangle pr-1 text-sm"></i
        >{{ $t("str.error.localApiError") }}
      </div>
    ...
    <div class="right">
      ...
      >
        <span class="tooltiptext top-bottom-right">
          <span v-if="isUpdatedVersion"><i class="fas fa-check"></i></span
          >{{ versionTooltip }}
        </span>
...
export default {
  ...
  mounted() {
    ...
    console.log(
      `Current Route: ${this.$route.name}, ${window.location.pathname}`
    );
    ...
    try {
      ...
      if (!this.IS_ELECTRON) {
        ...
        let cardLocale = window.localStorage.getItem(
          "lmt-settings-card-locale"
        );
  ...
  methods: {
    ...
    setCurrentPage(page) {
      ...
      var label =
        "From [" + pagekeys[this.currentPage] + "] to [" + pagekeys[page] + "]";
