<template>
  ...
  <div id="content">
    ...
    <div class="px-0 tabs xxs:px-1 xs:px-2" v-if="!isLoading">
      <div class="tab-title-group">
        <div class="text-sm tab-title xxs:text-base xs:text-lg" @click="switchTab(TABS.oppo)" :class="{ active: isShowOppo }">
          <i class="fas fa-swords"></i>
        </div>
        <div class="text-sm tab-title xxs:text-base xs:text-lg" @click="switchTab(TABS.oppog)" :class="{ active: isShowOppoGrave }">
          <i class="fas fa-tombstone-alt"></i>
        </div>
      </div>
      <div class="tab-title-group">
        <div class="text-sm tab-title xxs:text-base xs:text-lg" @click="switchTab(TABS.my)" :class="{ active: isShowMy }">
          <i class="fas fa-user-cowboy"></i>
        </div>
        <div class="text-sm tab-title xxs:text-base xs:text-lg" @click="switchTab(TABS.myg)" :class="{ active: isShowMyGrave }">
          <i class="fas fa-tombstone-alt"></i>
        </div>
      </div>
    </div>
    ...
    <div class="layerpanel max-w-[280px] h-10 p-2 xxs:h-11 xxs:p-1" :class="{ expanded: currentLayer != LAYERS.base }" v-if="isShowOppo">
      <button v-if="currentLayer == LAYERS.base" @click="onOpenDecklib" class="px-2 py-1 text-sm btn btn-decklib xxs:text-base">
        Open Deck Library
      </button>
