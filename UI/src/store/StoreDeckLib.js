  ...
  actions: {
    initStore() {
      ...
      if (window.ipcRenderer) {
        ...
        // Sample data
        this.decks = JSON.parse(
          window.localStorage.getItem(`lmt-settings-${storeid}`)
        );
    ...
    updateStore() {
      if (window.ipcRenderer) {
        window.ipcRenderer.send(
          "save-store",
          "deck-lib",
          JSON.stringify(this.decks, null, "\t")
        );
      } else {
        window.localStorage.setItem(
          `lmt-settings-${storeid}`,
          JSON.stringify(this.decks)
        );
      }
    },
    processPaste(deckCode) {
      ...
      try {
        ...
        while (
          this.decks.findIndex((item) => item.title == newTitle) != -1 &&
          i < 1000
        ) {
          i += 1;
